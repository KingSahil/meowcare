...
import { createMedicine, fetchBurnoutSuggestion, fetchDashboardData, syncMedicineStatus } from '@/lib/api';
...
export const useCareStore = create<CareStore>((set, get) => ({
  ...
  updateMedicineStatus: async (id, status) => {
    if (status === 'taken') {
      await syncMedicineStatus('taken');
    }

    const medicines = get().medicines.map((medicine) => (medicine.id === id ? { ...medicine, status } : medicine));
    set({ medicines });
